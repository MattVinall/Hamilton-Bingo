...
const BingoBoard: React.FC<Props> = ({ businesses }) => {
    ...
    return (
        <>
            <div id="bingoBoard" className="md:rounded-lg p-2 md:p-4 min-h-[80vh] sm:min-h-[80vh] md:min-h-[80vh] xl:min-h-[90vh]! 2xl:min-h-[90vh]">
                    ...
                    {Object.keys(bingoBoard).map((key, index) => (
                        <div key={key + index} className="col-span-1">
                            ...
                            {bingoBoard[key as keyof BingoBoard].map((business: Business) => (
                                <div key={`${key}-${business.id}`} className="card w-full flex flex-col items-center justify-center h-[25%] md:h-[50%] px-2 md:px-2 text-xs my-1 md:my:4 bg-navy">
